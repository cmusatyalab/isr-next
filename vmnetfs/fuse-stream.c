        ...
        struct vmnetfs_image *img)
{
    ...
    _vmnetfs_fuse_add_file(streams, "chunks_uploaded", &stream_ops,
            _vmnetfs_bit_get_stream_group(img->uploaded_map));
...
void _vmnetfs_fuse_stream_populate_root(struct vmnetfs_fuse_dentry *dir,
        struct vmnetfs *fs)
{
    _vmnetfs_fuse_add_file(dir, "log", &stream_ops,
            _vmnetfs_log_get_stream_group(fs->log));
}
